...
; success: returns a pointer to a news string (exact copy of the source)
...
_len:                           ;; calculates the len of the source, adds one for null terminator ;;
   ...
   ; mov     rcx, rax            ; saving the len to rcx general purpose register
   ; inc     rcx                 ;increment result for one to make space for /0
   inc      rax

_malloc:
    mov     rdi, rax            ;rcx at this point - len of the required memory
    call    malloc              ;malloc - takes rdi (len) as argument
    cmp     rax, 0              ;protect malloc - check if zero
    je      _error              ; malloc fails == jump to _error routine

_copy:
    mov     rdi, rax            ;rax at this point - pointer to malloced memory
